...
void inserareSfarsit(Nod** cap, Nod** coada, Carte c)
{
	Nod* nou = (Nod*)malloc(sizeof(Nod));
	nou->info = c;
	nou->next = NULL;
...
void inserareInceputCircular(Nod** cap,Nod** coada,Carte c)
{
	Nod* nou = (Nod*)malloc(sizeof(Nod));
	nou->info = c;
	if ((*cap) == NULL)
...
//dr st
void afisareInversaLista(Nod* coada)
{
	while (coada != NULL)
	{
...
Nod* cautareDupaTitlu(Nod* cap, const char* titlu)
{
	while (cap != NULL)
	{
...
int ldiToVector(Nod* cap, Carte v[])
{
	int n = 0;
	while (cap != NULL)
	{
...
void ldiToLsi(Nod* cap, Nod** lsi, Nod** coada)
{
	*lsi = *coada = NULL;
	while (cap != NULL)
	{
