...
void inseareLaInceput(Nod** cap, Carte c)
{
...
void inseareDupaPret(Nod** cap, Carte c)
{
	Nod* nou = (Nod*)malloc(sizeof(Nod));
	nou->info = c;
	nou->next = NULL;
...
Nod* stergerNod(Nod* cap, char* titlu_c)
{
	Nod* aux = cap;
	if (strcmp(cap->info.titlu, titlu_c) == 0)
	{
		aux = cap->next;
		free(cap->info.titlu);
		free(cap);
	...
	while (aux->next != NULL)
	{
		if (strcmp(aux->next->info.titlu, titlu_c) == 0)
		{
			Nod* sters = aux->next;
			aux->next = sters->next;
...
Nod* steregerPoz(Nod* cap, int poz)
{
	Nod* aux = cap;
	if (cap && poz == 0)
	{
...
static Carte citireCarte(FILE *file)
{
	char buffer[30];
	Carte c;
...
Nod* citireListaCarti(const char* numeFisier)
{
	...
	Nod* cap = NULL;
	inseareLaInceput(&cap, citireCarte(file));
	fclose(file);
	return cap;
}
...
//la circulara mereu fac *aux=cap si dupa verific cap!=aux